...
const VendorDetail: React.FC = () => {
  ...
  const handleEditEntry = (entry: VendorLedgerEntry) => {
    ...
    // Ensure units is an array with the correct length
    let unitsArray: string[] = [];
    if (entry.units) {
      if (Array.isArray(entry.units)) {
        unitsArray = entry.units;
      } else if (typeof entry.units === 'string') {
        unitsArray = entry.units.split(',');
      }
    }
    
    // Ensure units array matches the length of descriptions
    while (unitsArray.length < (entry.descriptions?.length || 0)) {
      unitsArray.push('meter');
    }
    ...
    setNewEntry({
      ...
      chequeNumber: entry.payment_method === 'cheque' ? entry.cheque_number : '',
  ...
  const exportToExcel = () => {
    ...
    const excelData = filteredEntries.flatMap(entry =>
      (entry.descriptions || []).map((desc, rowIndex) => ({
        ...
        Unit: Array.isArray(entry.units) ? entry.units[rowIndex] || 'meter' : 'meter',
        'Price per Unit': entry.prices?.[rowIndex] || '',
  ...
  const handleFormSubmit = () => {
    ...
    try {
      ...
      // Ensure units array matches the length of cleanedDescriptions
      ...
      const entryData = {
        ...
        cheque_number: newEntry.paymentMethod === 'cheque' ? newEntry.chequeNumber : '',
  ...
  const handleChange = (field: keyof Pick<NewLedgerEntry, 'descriptions' | 'quantities' | 'price_per_meter' | 'units'>, index: number, value: string) => {
    setNewEntry((prev) => {
      const copy = { ...prev };
                      ...
                      <TableRow key={`${entry.id}-${rowIndex}`}>
                        ...
                        >
                          {Array.isArray(entry.units) 
                            ? entry.units[rowIndex] || 'meter'
                            : typeof entry.units === 'string' && entry.units.split(',')[rowIndex] || 'meter'}
                        </TableCell>
                        <TableCell
                          style={{ ...Fonts.Roboto, fontSize: isMobile ? '12px' : '14px' }}
                        >
